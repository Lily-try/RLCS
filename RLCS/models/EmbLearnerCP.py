...
class Contra(nn.Module):
    ...
    def sim(self, x, y):
        return F.cosine_similarity(x.unsqueeze(0), y.unsqueeze(0)).clamp(min=1e-5, max=1.0 - 1e-5)
    ...
    def forward(self, h, h_aug, tau, train, alpha, lam, edge_index):
        ...
        if len(pos) !=0:
            ...
            #cploss
            c_n_pos = h[mask_p].mean(dim=0)
            c_k_pos = h_aug[mask_p].mean(dim=0)
            c_k_neg = h_aug[~mask_p].mean(dim=0)

            sim_pos_nk = self.sim(c_n_pos, c_k_pos)
            sim_neg_nk = self.sim(c_n_pos, c_k_neg)
            s_cp_1 = -torch.log(sim_pos_nk) - torch.log(1 - sim_neg_nk)

            sim_pos_kn = self.sim(c_k_pos, c_n_pos)
            sim_neg_kn = self.sim(c_k_pos, c_k_neg)
            s_cp_2 = -torch.log(sim_pos_kn) - torch.log(1 - sim_neg_kn)

            inter_proto_loss = 0.5 * (s_cp_1 + s_cp_2)
        ...
        loss = (total_intra_loss + alpha * inter_proto_loss) + lam * loss_unsup
        return loss

class EmbLearnerCP(nn.Module):

    def __init__(self, node_in_dim, hidden_dim, num_layers, dropout, tau, device, alpha, lam, k,use_hypergraph=True):
        super(EmbLearnerCP, self).__init__()
        ...
        if use_hypergraph:
            self.aug_query_layers = nn.ModuleList()
    ...
    def compute_loss(self,train):
        ...
        if self.use_hypergraph:
            h_augq = F.relu(self.aug_query_layers[0](querys, edge_index_aug))
            h_aug = F.relu(self.aug_layers[0](feats, edge_index_aug))
        ...
        if self.use_hypergraph:
            atten_coh_ = torch.cat([self.aug_first_q_att_layer(querys), self.aug_first_att_layer(feats)],1)  # atten_coh_=（115，2）
